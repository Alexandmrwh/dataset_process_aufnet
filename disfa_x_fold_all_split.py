...
Disfa10FoldsPath = "../"
DisfaImagePath = "~/AU-WorkSite/FAC_DataBase/DISFA/dataset_process_0325_hanwu/align_crop_disfa/"
...
def splitnew(splits_in, au, audict, splitres, foldnum):
    ...
    aures_splits = partition(ausubs_res, foldnum)
...
def generate10splits(expname):
    ...
    splitres = np.zeros(shape=[aus+1, foldnum+2], dtype=int)
    
    # split au5
    au5dict = subdictlist[au_idx.index(5)]
    au5subs = list(au5dict.keys()) 
    au5splits = partition(au5subs, foldnum)
    for i in range(foldnum):
    ...
    for idx in au_split_seq:
        if idx == 9:
            ausplits_new, splitres_new = splitnew(au5splits, 9, subdictlist[au_idx.index(9)], splitres, foldnum)
        else: 
            ausplits_new, splitres_new = splitnew(ausplits_new, idx, subdictlist[au_idx.index(idx)], splitres_new, foldnum)
    
    splitres_new[aus] = splitres_new.sum(axis=0)
    for i in range(aus):
        splitres_new[i][foldnum+1] = sum(splitres_new[i][:])
        splitres_new[i][foldnum] = sum(appeartimes[i])  
        ...
        for session_idx in sessions:
            ...
            if os.path.isfile(session_label_path):
                ...
                for i in range(0, sample_num - stride * samplingrate + 1, stride * samplingrate):
                    ...
                    '''
                    aus = aus.astype(np.int)
                    if i + samplingrate < sample_num:
                        line2=all_samples[i+samplingrate]
                        frameIdx2, _, _, _, _, _, \
